...
/**
 * ─────────────────────────────────────────────────────────
 * ชื่อไฟล์: useLocalStorageState.ts
...
export function loadStored<T>(key: string, fallback: T, validate?: (v: any) => boolean): T {
  try {
...
export function useLocalStorageState<T>(
  ...
  const [value, setValue] = useState<T>(() => loadStored(key, fallback, validate));
