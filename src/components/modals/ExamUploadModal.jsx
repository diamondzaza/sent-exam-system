    ...
    return (<div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={onClose} className="text-slate-400 hover:bg-slate-800 hover:text-white" aria-label="ปิดหน้าต่าง">
            <X className="w-5 h-5"/>
          </Button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6 space-y-6 flex-1">
          ...
          {isReupload && (<div className="bg-amber-50 border border-amber-300 text-amber-900 p-3 rounded-xl text-xs flex items-start space-x-2">
          ...
          <div>
            ...
            e.preventDefault();
            setIsDragging(true);
        }} onDragLeave={() => setIsDragging(false)} onDrop={handleFileDrop} className={`border-2 border-dashed rounded-xl p-6 text-center transition-all ${isDragging
          ...
          <div>
            ...
            return (<button type="button" key={mat} onClick={() => toggleMaterial(mat)} className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-colors ${isSelected
            ...
            <div className="flex space-x-2">
              <Input type="text" placeholder="ระบุสิ่งที่อนุญาตเพิ่มเติม" value={newMaterialInput} onChange={(e) => setNewMaterialInput(e.target.value)} onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
