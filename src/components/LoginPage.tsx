            ...
            <form onSubmit={handleCustomLogin} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <Input
                  id="login-username"
                  type="text"
                  placeholder="เช่น somchai.t หรือ av.staff1"
                  value={username}
            ...
            <div className="border-t border-slate-200 pt-4 space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {users.map((user) => {
                  ...
                  if (user.role === 'AudioVisual') shortRole = 'โสตฯ';
                  if (user.role === 'Operations') shortRole = 'ฝ่ายจัดสอบ';
                  if (user.role === 'Admin') shortRole = 'แอดมิน';

                  return (
                    <button
                      key={user.id}
                      onClick={() => onLogin(user)}
                      className="p-2.5 rounded-xl border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/40 text-left transition-all flex items-center space-x-2.5 group"
                    >
