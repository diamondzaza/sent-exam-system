...
export const OperationsView: React.FC<OperationsViewProps> = ({
  ...
  const handleConfirmReceipt = (exam: ExamEntity) => {
    onUpdateExamStatus(
      exam.E_No,
      'READY_FOR_EXAM',
      `ฝ่ายดำเนินการสอบ (${currentUser.name}) รับมอบซองข้อสอบและจัดเก็บในตู้นิรภัยห้องมั่นคงเรียบร้อย`
    );
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <Input
            type="text"
            placeholder="ค้นหารหัสวิชา, ชื่อวิชา, ห้องสอบ, อาจารย์..."
            value={searchQuery}
      ...
      <Card className="overflow-hidden">
        <CardHeader className="flex-row items-center justify-between space-y-0 border-b border-slate-200 bg-slate-50/60 rounded-b-none">
          ...
          <Button
            ...
            variant="secondary"
            size="sm"
            className="no-print shrink-0 border border-slate-300"
          >
                  ...
                  <div key={exam.E_No} className="p-6 hover:bg-slate-50/50 transition-colors space-y-4">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {canConfirmReceipt && (
                          <Button
                            onClick={() => handleConfirmReceipt(exam)}
                            size="sm"
                        ...
                        {isReady && (
                          <Badge variant="success" className="px-3 py-1.5 rounded-xl text-xs gap-1.5">
