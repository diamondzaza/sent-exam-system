...
import { ExamEntity, UserAccount, ExamStatus, CourseEntity } from '../types.ts';
...
interface AudioVisualViewProps {
  currentUser: UserAccount;
  courses?: CourseEntity[];
  exams: ExamEntity[];
  onPreviewExam: (exam: ExamEntity) => void;
  onOpenEnvelope: (exam: ExamEntity) => void;
  onUpdateExamStatus: (examNo: string, newStatus: ExamStatus, note?: string) => void;
  onPrintExam: (exam: ExamEntity) => void;
}

export const AudioVisualView: React.FC<AudioVisualViewProps> = ({
  currentUser,
  courses = [],
  exams,
  onPreviewExam,
  ...
  const [rejectingExam, setRejectingExam] = useState<ExamEntity | null>(null);
  const [rejectReason, setRejectReason] = useState('');

  // 1. Build unified subject and instructor directory
  interface SubjectInstructorItem {
    courseId: string;
    courseName: string;
    teacherName: string;
    teacherTel: string;
  ...
  const subjectList: SubjectInstructorItem[] = (courses.length > 0 ? courses : []).map((c) => {
  ...
  const filteredSubjects = subjectList.filter((item) => {
    ...
    const matchesStatus =
      statusFilter === 'ALL' || (item.exam ? item.exam.status === statusFilter : false);
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1">
          ...
          <Input
            type="text"
            placeholder="ค้นหารหัสวิชา ชื่อวิชา หรืออาจารย์ผู้สอน"
            value={searchQuery}
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        {item.exam ? (
                          ...
                          <span className="text-slate-400">{item.studentCount} คน</span>
                        )}
                      </td>
                          ...
                          {item.exam ? (
                            <>
                              <Button
                                ...
                                className={isSelected ? '' : 'text-indigo-700 border-indigo-200 hover:bg-indigo-50'}
                                onClick={() => setSelectedCourseId(item.courseId)}
                              >
                              ...
                              <Button
                                ...
                                onClick={() => item.exam && onPreviewExam(item.exam)}
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className="size-7 text-purple-600 hover:bg-purple-50 hover:text-purple-700"
                                onClick={() => item.exam && onOpenEnvelope(item.exam)}
                                title="พิมพ์ใบปะหน้าซองข้อสอบ"
                              >
        ...
        <div className="divide-y divide-slate-200">
          {filteredExams.length === 0 ? (
            ...
            filteredExams.map((exam) => {
              ...
              return (
                <div key={exam.E_No} className="p-6 hover:bg-slate-50/50 transition-colors space-y-4">
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="space-y-1.5 flex-1">
                      ...
                      {exam.envelope_notes && (
                        <div className="text-[11px] text-amber-900 bg-amber-50/70 p-2 rounded-lg border border-amber-200 mt-1">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                      <Button
                        ...
                        title="เปิดดูตัวอย่างข้อสอบพร้อมลายน้ำ"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="text-indigo-800 border-indigo-200 hover:bg-indigo-50"
                      ...
                      <Button
                        ...
                        title="สั่งพิมพ์ข้อสอบจริงเข้าเครื่องพิมพ์"
                      >
                  ...
                  <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2 text-xs">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {exam.status === 'VERIFIED' && (
                        <Button
                          size="sm"
                          onClick={() => handleStartPrinting(exam)}
                        >
                          <Printer className="w-3.5 h-3.5" />
                      ...
                      {exam.status === 'PRINTING' && (
                        <Button
                          size="sm"
                          className="bg-purple-600 hover:bg-purple-700 text-white"
                          onClick={() => handleFinishPrinting(exam)}
                        >
                      ...
                      {exam.status === 'PRINTED' && (
                        <Button
                          ...
                          onClick={() => handleDeliverToOps(exam)}
                        >
                      ...
                      {['DELIVERED_OD', 'READY_FOR_EXAM'].includes(exam.status) && (
            ...
            <form onSubmit={handleConfirmReject} className="p-6 space-y-4 text-xs">
              ...
              <div>
                ...
                <Textarea
                  ...
                  required
                  placeholder="เช่น ไฟล์ PDF ไม่สมบูรณ์, หน้าที่ 4 ข้อสอบตกหล่น, ขอให้อัปโหลดใหม่..."
                />
              </div>
