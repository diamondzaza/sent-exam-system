...
import { CourseEntity, ExamEntity, UserAccount } from '../types.ts';
...
interface TeacherViewProps {
  ...
  exams: ExamEntity[];
  onOpenUploadModal: (course: CourseEntity, existingExam?: ExamEntity | null, isReupload?: boolean) => void;
  onPreviewExam: (exam: ExamEntity) => void;
  onRemoveExam: (examNo: string) => void;
  onAddNewCourse: (newCourse: CourseEntity) => void;
  onOpenEnvelope: (exam: ExamEntity) => void;
}
...
export const TeacherView: React.FC<TeacherViewProps> = ({
  ...
  const [newCourseStudents, setNewCourseStudents] = useState<number>(45);
  const [newCourseError, setNewCourseError] = useState('');

  // Filter courses for this teacher or all if academic affairs
  const myCourses = courses.filter(
    (c) => c.teacher_id === currentUser.id || currentUser.id === 'T001'
  );
  ...
  const getExamForCourse = (courseId: string) => {
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3">
            <Button
              ...
              className="border border-indigo-400/40 shadow-sm"
            >
              <Plus />
          ...
          <p className="text-2xl font-bold text-purple-600 mt-1">
            {
              exams.filter(
                (e) =>
                  myCourses.some((c) => c.Course_id === e.Subject_ID) &&
                  ['PRINTING', 'PRINTED', 'DELIVERED_OD', 'READY_FOR_EXAM'].includes(e.status)
              ).length
            }{' '}
            วิชา
          </p>
          ...
          <p className="text-2xl font-bold text-emerald-600 mt-1">
            {
              exams.filter(
                (e) =>
                  myCourses.some((c) => c.Course_id === e.Subject_ID) &&
                  ['DELIVERED_OD', 'READY_FOR_EXAM'].includes(e.status)
              ).length
            }{' '}
            วิชา
          </p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => setShowNewCourseModal(true)}
              className="border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 hover:text-indigo-700"
            >
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  {statusConfig ? (
                    ...
                    <Badge variant="warning">ยังไม่ส่งข้อสอบ</Badge>
                  )}
                </div>
                ...
                <div className="text-[11px] text-slate-500 flex items-center justify-between pt-2 border-t border-slate-100">
                  ...
                  {isSelected ? (
                    <span className="text-indigo-700 font-bold flex items-center space-x-1 text-[11px]">
            ...
            <div className="flex items-center space-x-2">
              ...
              <h3 className="font-display font-bold text-base text-slate-900">
                {selectedCourseId === 'ALL'
                  ...
                  : `รายการข้อสอบและการดำเนินการ: ${selectedCourseId} ${
                      myCourses.find((c) => c.Course_id === selectedCourseId)?.Course_Name || ''
                    }`}
              </h3>
            </div>
            <p className="text-xs text-slate-500">
        ...
        <div className="divide-y divide-slate-200">
          {displayedCourses.length === 0 ? (
            ...
            displayedCourses.map((course) => {
              ...
              return (
                <div key={course.Course_id} className="p-6 hover:bg-slate-50/50 transition-colors space-y-4">
                  {/* Course Header Row */}
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {!exam ? (
                        <Button onClick={() => onOpenUploadModal(course, null, false)}>
                          <Upload />
                        ...
                        <>
                          <Button
                            ...
                            title="ดูตัวอย่างข้อสอบและลายน้ำความปลอดภัย"
                          >
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => onOpenEnvelope(exam)}
                            title="ดูและสั่งพิมพ์ใบปะหน้าซองข้อสอบ"
                          >
                          ...
                          {['SUBMITTED', 'VERIFIED', 'REJECTED'].includes(exam.status) && (
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => onOpenUploadModal(course, exam, true)}
                              className="border-amber-300 bg-amber-50 text-amber-800 hover:bg-amber-100 hover:text-amber-800"
                              title="อัปโหลดไฟล์ข้อสอบฉบับปรับปรุงใหม่"
                            >
                              <RefreshCw />
                          ...
                          {['SUBMITTED', 'VERIFIED', 'REJECTED'].includes(exam.status) && (
                            <Button
                              ...
                              title="ยกเลิกการส่งข้อสอบฉบับนี้"
                            >
                              <Trash2 />
                              <span>ยกเลิกส่ง</span>
                            </Button>
                  ...
                  {!exam && (
                    <div className="bg-amber-50/60 border border-amber-200 rounded-xl p-4 text-xs text-amber-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="space-y-1">
                      ...
                      <Button
                        onClick={() => onOpenUploadModal(course, null, false)}
                        className="shrink-0"
                      >
                        <Upload />
                  ...
                  {exam && (
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/80 space-y-3">
                      ...
                      {exam.status === 'REJECTED' && exam.rejection_reason && (
                        <div className="text-[11px] text-rose-800 bg-rose-50 p-2.5 rounded-lg border border-rose-200 flex items-start space-x-2">
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full border border-slate-200 overflow-hidden">
            ...
            <form onSubmit={handleCreateCourse} className="p-6 space-y-4 text-xs">
              {newCourseError && (
                <div className="bg-rose-50 border border-rose-300 text-rose-800 px-3 py-2 rounded-lg flex items-center space-x-1.5">
              ...
              <div>
                ...
                <Input
                  type="text"
                  placeholder="เช่น CS345, MA205, SC102"
                  value={newCourseCode}
                  onChange={(e) => {
              ...
              <div>
                ...
                <Input
                  type="text"
                  placeholder="เช่น ปัญญาประดิษฐ์ (Artificial Intelligence)"
                  value={newCourseName}
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewCourseStudents(parseInt(e.target.value) || 1)}
                  />
                </div>
              </div>
